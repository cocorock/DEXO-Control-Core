...
from exoskeleton_control.msg import DualAnkleTrajectory, ExoskeletonState, MotorStatus, Torques, EStopTrigger, Trigger, FSMState
...
class MotorControlNode:
    def __init__(self):
        rospy.init_node('motor_control_node_4motors')
        ...
        # Motor state storage (ordered by motor ID) - 4 motors now
        ...
        # Desired trajectory storage - for ankle positions and velocities
        self.desired_ankle_positions = np.zeros((2, 2))  # [right_ankle, left_ankle] x [x, y]
        self.desired_ankle_velocities = np.zeros((2, 2))  # [right_ankle, left_ankle] x [vx, vy]
        
        # Leg parameters for inverse kinematics
        self.L1 = 0.425  # Thigh length (m)
        self.L2 = 0.45   # Shin length (m)

        # Feedforward torque calculator for both legs
        ...
        # Subscribers - Change to DualAnkleTrajectory
        rospy.Subscriber('dual_ankle_trajectory', DualAnkleTrajectory, self.dual_ankle_trajectory_callback)
        ...
        rospy.loginfo("m: Motor Control Node (4 motors) initialized. Waiting for calibration...")
        ...
        for motor_id, config in self.motor_configs.items():
            ...
            rospy.loginfo(f"m:     Joint limits: {math.degrees(config.min_angle_rad):.1f}° to {math.degrees(config.max_angle_rad):.1f}°")
            rospy.loginfo(f"m:     Motor direction: {config.direction}")
    ...
    def load_configuration(self):
        ...
        try:
            ...
            self.calibration_sequence = calibration_config.get('sequence', [1, 2, 3, 4])
            ...
            self.torque_thresholds = calibration_config.get('torque_thresholds', [3.0, 1.0, 3.0, 1.0])
            ...
            self.compensation_torques = {
                ...
                'left_hip': compensation_torques_config.get('left_hip', [0.0, 0.0]),
                'left_knee': compensation_torques_config.get('left_knee', [0.0, 0.0])
            }

            # Load motor hardware configuration for 4 motors
            motors_config = rospy.get_param('~motors', {})
            self.motor_config_params = {
                'motor_ids': motors_config.get('ids', [0x06, 0x08, 0x07, 0x09]),
                'joint_names': motors_config.get('joint_names', ["right_hip", "right_knee", "left_hip", "left_knee"]),
                'motor_models': motors_config.get('models', ["AK80_64", "AK80_8", "AK80_64", "AK80_8"]),
                'angle_limits_deg': motors_config.get('angle_limits_deg', [[-50, 90], [-100, 10], [-50, 90], [-100, 10]]),
                'motor_directions': motors_config.get('directions', [1, 1, 1, 1])
            }

            # Load control gains from new structure for 4 motors
            ...
            left_hip_gains = control_gains_config.get('left_hip', {
                'calibration': {'kp': 0.0, 'kd': 5.0},
                'hold': {'kp': 2.0, 'kd': 1.5},
                'trajectory': {'kp': 5.0, 'kd': 1.0}
            })
            left_knee_gains = control_gains_config.get('left_knee', {
                'calibration': {'kp': 0.0, 'kd': 5.0},
                'hold': {'kp': 2.0, 'kd': 1.5},
                'trajectory': {'kp': 5.0, 'kd': 1.0}
            })
            self.motor_gains_config = [right_hip_gains, right_knee_gains, left_hip_gains, left_knee_gains]
    ...
    def set_default_configuration(self):
        ...
        self.calibration_sequence = [1, 2, 3, 4]
        self.calibration_speed = math.radians(60.0)
        self.max_calibration_time = 30.0
        self.torque_thresholds = [3.0, 1.0, 3.0, 1.0]
        
        # Per-motor control gains for 4 motors
        self.motor_gains_config = [
            ...
            {'calibration': {'kp': 0.0, 'kd': 5.0}, 'hold': {'kp': 2.0, 'kd': 1.5}, 'trajectory': {'kp': 5.0, 'kd': 1.0}},
            # Default for motor 3 (left hip)
            {'calibration': {'kp': 0.0, 'kd': 5.0}, 'hold': {'kp': 2.0, 'kd': 1.5}, 'trajectory': {'kp': 5.0, 'kd': 1.0}},
            # Default for motor 4 (left knee)
            {'calibration': {'kp': 0.0, 'kd': 5.0}, 'hold': {'kp': 2.0, 'kd': 1.5}, 'trajectory': {'kp': 5.0, 'kd': 1.0}}
        ]

        # Motor configurations - 4 motors
        self.motor_config_params = {
            'motor_ids': [0x06, 0x08, 0x07, 0x09],  # Right hip, Right knee, Left hip, Left knee
            'joint_names': ["right_hip", "right_knee", "left_hip", "left_knee"],
            'motor_models': ["AK80_64", "AK80_8", "AK80_64", "AK80_8"],
            'angle_limits_deg': [[-50, 90], [-100, 10], [-50, 90], [-100, 10]],
    ...
    def calculate_inverse_kinematics(self, x, y):
        ...
        if distance > max_reach:
            rospy.logwarn_throttle(1.0, f"m: Target unreachable: distance={distance:.3f}, max_reach={max_reach:.3f}")
            return None, None
        
        if distance < min_reach:
            rospy.logwarn_throttle(1.0, f"m: Target too close: distance={distance:.3f}, min_reach={min_reach:.3f}")
            return None, None
    ...
    def dual_ankle_trajectory_callback(self, msg):
        """Process dual ankle trajectory commands."""
        ...
        # Extract ankle positions and velocities
        self.desired_ankle_positions[0, :] = [msg.right_ankle_pos_x, msg.right_ankle_pos_y]  # Right ankle
        self.desired_ankle_positions[1, :] = [msg.left_ankle_pos_x, msg.left_ankle_pos_y]   # Left ankle
        self.desired_ankle_velocities[0, :] = [msg.right_ankle_vel_x, msg.right_ankle_vel_y]  # Right ankle
        self.desired_ankle_velocities[1, :] = [msg.left_ankle_vel_x, msg.left_ankle_vel_y]   # Left ankle
        
        # Log trajectory data for debugging
        if self.debug_flag:
            rospy.loginfo_throttle(1.0, f"m: Received ankle trajectory: "
                                 f"R_ankle=({msg.right_ankle_pos_x:.3f}, {msg.right_ankle_pos_y:.3f}), "
                                 f"L_ankle=({msg.left_ankle_pos_x:.3f}, {msg.left_ankle_pos_y:.3f})")
            ...
            for direction in [-1, 1]:  # negative first, then positive (standard order)
                ...
                while True:
                    ...
                    if motor_driver.read_motor_status(self.can_channel, controller, state, 
                        ...
                        # Log current position and torque for debugging (use direction-corrected position)
                        corrected_position_for_log = state.p_out * config.direction
                        rospy.loginfo_throttle(0.5, f"m:   Motor {motor_id} pos: {math.degrees(corrected_position_for_log):.1f}°, "
                        ...
                        # Apply direction correction to raw motor position for comparison with logical limits
                        corrected_position = state.p_out * config.direction
                        safety_margin_rad = math.radians(5.0)  # 5 degree safety margin
                        limit_exceeded = False
                        
                        if direction > 0 and corrected_position > (config.max_angle_rad - safety_margin_rad):
                            rospy.logwarn(f"m:   Motor {motor_id} position {math.degrees(corrected_position):.1f}° approaches max limit "
                                         f"{math.degrees(config.max_angle_rad):.1f}° in positive direction")
                            limit_exceeded = True
                        elif direction < 0 and corrected_position < (config.min_angle_rad + safety_margin_rad):
                            rospy.logwarn(f"m:   Motor {motor_id} position {math.degrees(corrected_position):.1f}° approaches min limit "
                        ...
                        if abs(state.t_out) > dynamic_threshold:
                            ...
                            limits.append(state.p_out)  # Store raw motor position as physical limit
                            corrected_pos_for_log = state.p_out * config.direction
                            rospy.loginfo(f"m:   Limit found at position: {math.degrees(corrected_pos_for_log):.1f}° (corrected), torque: {state.t_out:.2f} Nm")
                ...
                if len(limits) == 1:  # After finding first limit, before second direction
                    ...
                    motor_driver.read_motor_status(self.can_channel, controller, state, 
                                                 max_attempts=3, timeout_ms=50, debug_flag=self.debug_flag)
                    corrected_eq_pos = state.p_out * config.direction
                    rospy.loginfo(f"m:   Equilibrium position: {math.degrees(corrected_eq_pos):.1f}° (corrected)")
    ...
    def send_motor_commands(self):
        ...
        try:
            with self.motor_lock:
                for i, motor_id in enumerate([1, 2, 3, 4]):  # All 4 motors
                    controller = self.motor_controllers[motor_id]
                    state = self.motor_states[motor_id]
                    config = self.motor_configs[motor_id]
                    ...
                    if self.trajectory_active and config.is_calibrated:
                        ...
                        leg_idx = 0 if i < 2 else 1  # 0 for right leg (motors 1,2), 1 for left leg (motors 3,4)
                        joint_idx = i % 2  # 0 for hip, 1 for knee
                        
                        # Get ankle position and velocity for this leg
                        ankle_pos = self.desired_ankle_positions[leg_idx, :]
                        ankle_vel = self.desired_ankle_velocities[leg_idx, :]
                        
                        # Calculate inverse kinematics
                        theta_hip, theta_knee = self.calculate_inverse_kinematics(ankle_pos[0], ankle_pos[1])
                        
                        if theta_hip is not None and theta_knee is not None:
                            # Use calculated joint angles
                            desired_angles = [theta_hip, theta_knee]
                            desired_pos = desired_angles[joint_idx]
                            
                            # Calculate joint velocities using Jacobian (simplified - could be improved)
                            # For now, use a simple approximation
                            joint_vel_scale = 0.1  # Simple scaling factor
                            desired_vel = joint_vel_scale * (ankle_vel[0] + ankle_vel[1])  # Simplified
                        else:
                            # IK failed - use safe values
                            desired_pos = 0.0
                            desired_vel = 0.0
                        ...
                        desired_pos = np.clip(desired_pos, min_limit, max_limit)
                        ...
                        if self.debug_flag and motor_id == 1:  # Log for hip motor only to reduce spam
                            rospy.loginfo_throttle(0.5, f"m: Sending trajectory cmd to motor {motor_id}: "
                                                 ...
                                                 f"vel={desired_vel:.3f}")
                        
                        # Set impedance control parameters
                        state.p_in = desired_pos * config.direction  # Apply motor direction to position command
                        state.v_in = desired_vel * config.direction  # Apply motor direction
                        state.kp_in = config.gains['trajectory']['kp']
                        state.kd_in = config.gains['trajectory']['kd']
                        # TESTING: Feedforward torques calculated but not sent to motors
                        # state.t_in = self.feedforward_torques[i] * config.direction  # Apply direction to torque
                        state.t_in = 0.0  # Zero torque for testing feedforward calculation
                        ...
                        if motor_driver.read_motor_status(self.can_channel, controller, state, 
                            ...
                            # Update motor state values directly after reading response (apply direction inversion)
                            self.motor_positions[i] = state.p_out * config.direction
                            self.motor_velocities[i] = state.v_out * config.direction
                            self.motor_torques[i] = state.t_out * config.direction
